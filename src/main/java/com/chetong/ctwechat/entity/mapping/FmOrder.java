...
public class FmOrder extends BasesModel implements java.io.Serializable {
	...
	public String getOrderTypeLabel() {
		if (StringUtils.isEmpty(getReviewType())) {
			if (StringUtils.isEmpty(getOrderType())) {
				if(serviceId == 1l){
					if (subjectId == 1l) {
						orderTypeLabel = "查勘";
			...
			if ("1".equals(getReviewType())) {
				orderTypeLabel = "拖车";
			} else if ("2".equals(getReviewType())) {
				orderTypeLabel = "快修";
			} else if ("3".equals(getReviewType())) {
				orderTypeLabel = "送油";
			} else if ("4".equals(getReviewType())) {
				orderTypeLabel = "搭电";
			} else if ("5".equals(getReviewType())) {
				orderTypeLabel = "换胎";
			} else if ("6".equals(getReviewType())) {
				orderTypeLabel = "送水";
			}
		}
