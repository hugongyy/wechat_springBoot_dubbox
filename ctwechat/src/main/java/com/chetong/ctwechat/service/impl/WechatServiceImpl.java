...
import com.chetong.ctwechat.util.common.MobileUtils;
...
public class WechatServiceImpl implements WechatService {
	...
	public static List<String> spAreas = new ArrayList<String>();
//	static {
...
//	}
	...
	public String getAnBangCompanyUserId() {
		return anBangCompanyUserId;
	}
	...
	public WeChatResponseModel bindingopenId(String uname, String pwd, String openId) {
		...
		if (list == null || list.size() == 0) {
			...
			Long userId = ctUser.getId();
			String token = TokenUtils.createToken(userId, "wechat");
			String redisKey = "Token:wechat";
			RedissonUtils.setMapCacheValue(redisKey, userId.toString(), token, loginValidTime);
			response.setToken(token);

			if (uList != null && uList.size() > 0) {
				response.setCtUser(uList.get(0));
	 ...
	 */
	@Override
	public WeChatResponseModel cxOrdersStatisticsNum(String userId, String otherLike, String dealStat) {
		...
		areasMap.put("userId", userId);
		areasMap.put("authName", "微信管理");
		String chargeArea = commExeSqlDAO.queryForObject("custom_rank_mapper.queryUserAreaCodes", areasMap);
		
		
		String oldDealStat = dealStat;
		if ("20".equals(dealStat)) {
			dealStat = "04";
		}
		...
		if (anBangCompanyUserId.equals(userId)) {
			// 安邦保险总部.特殊,先判断
			param.put("userId", userId);
			mapOrder = commExeSqlDAO.queryForList("CT_WECHAT_INFO.countOrder4AnBang", param);
		} else if (chargeArea != null && !"".equals(chargeArea)) {
			userTypeLabel = "Leader";
			// 公司领导,区域总监.
			if (!"000000".equals(chargeArea)) {
				param.put("chargeArea", chargeArea.split(","));
			}
			mapOrder = commExeSqlDAO.queryForList("CT_WECHAT_INFO.countOrder4Leader", param);
		...
		if (mapOrder == null || mapOrder.size() <= 0) {
			...
			for (int i = 0; i < mapOrder.size(); i++) {
				if (oldDealStat != null && oldDealStat.equals(mapOrder.get(i).getState())) {
	...
	public WeChatResponseModel hyOrdersStatisticsNum(String userId, String otherLike) {
		...
		if (anBangCompanyUserId.equals(userId)) {
			// 安邦保险总部.特殊,先判断
			param.put("userId", userId);
			mapOrder = commExeSqlDAO.queryForList("CT_WECHAT_INFO.countHyOrder4AnBang", param);
		} else if (chargeArea != null && !"".equals(chargeArea)) {
			userTypeLabel = "Leader";
			// 公司领导,区域总监.
			if (!"000000".equals(chargeArea)) {
			...
			mapOrder = commExeSqlDAO.queryForList("CT_WECHAT_INFO.countHyOrder4Leader", param);
	...
	public WeChatResponseModel cxOrdersSelect(String userId, String workType, String otherLike, String dealStat,
		...
		if (anBangCompanyUserId.equals(userId)) {
			...
			list = commExeSqlDAO.queryForPage("CT_WECHAT_INFO.queryOrderList4AnBang", param, pageBounds);
		} else if (chargeArea != null && !"".equals(chargeArea)) {
			userTypeLabel = "Leader";
			// 公司领导,区域总监.
			if (!"000000".equals(chargeArea)) {
			...
			list = commExeSqlDAO.queryForPage("CT_WECHAT_INFO.queryOrderList4Leader", param, pageBounds);
		} else if ("0".equals(u.getUserType()) && "1".equals(u.getIsMgr())) {
			userTypeLabel = "PM";
			// 项目经理,使用所属的机构的userId
			group = (CtGroup) commExeSqlDAO.queryForObject("CT_WECHAT_INFO.queryGroupByPorjectManage", userId);
...
//			param.put("workType", workType); // 车童不分本异地订单.
	...
	public WeChatHyResponseModel hyOrdersSelect(String userId, String workType, String otherLike, String dealStat,
		...
		if (anBangCompanyUserId.equals(userId)) {
			...
			param.put("dealStat", dealStat);
			param.put("workType", workType);
			list = commExeSqlDAO.queryForPage("CT_WECHAT_INFO.queryHyOrderList4AnBang", param, pageBounds);
		} else if (chargeArea != null && !"".equals(chargeArea)) {
			userTypeLabel = "Leader";
			// 公司领导,区域总监.
			if (!"000000".equals(chargeArea)) {
			...
			param.put("dealStat", dealStat);
			param.put("workType", workType);
			list = commExeSqlDAO.queryForPage("CT_WECHAT_INFO.queryHyOrderList4Leader", param, pageBounds);
	...
	public ResultVO<String> getAccessToken() {
		ResultVO<String> response = new ResultVO<String>();
		if (TokenThread.access == null || "".equals(TokenThread.access)) {
			response.setResultCode("fail");			
			response.setResultMsg("参数为空,获取失败");
			return response;
		} else {
			response.setResultCode("success");
			response.setResultObject(TokenThread.access);
			response.setResultMsg("获取accessToken成功");
			return response;
		}
	}
	...
	public QueryOrderBaseBean getRelationOrders(Long userId, String caseNo, String token) {
		...
		for (FmOrder o : orderList) {
			...
			if (ycOrderRelMap != null && ycOrderRelMap.get("id") != null) {
				...
				if ("0".equals(o.getOrderType())) {
					...
					url = headerUrl + "apph5/#/yc/survey?userId=" + o.getSellerUserId() + "&orderNo=" + o.getOrderNo()
							+ "&reportNo=" + o.getCaseNo() + "&workId=" + ycOrderRelMap.get("workId")+"&token=" + token;
			...
			String linkTel = MobileUtils.fuzzyPhone(o.getLinkTel());
			o.setLinkTel(linkTel);
			
			String delegateDesc = MobileUtils.fuzzyPhoneOfText(o.getDelegateDesc());
			o.setDelegateDesc(delegateDesc);
	...
	@Override
	public List<String> querySpAreas() {
		Map<String, Object> map = new HashMap<String, Object>();
		return commExeSqlDAO.queryForList("CT_WECHAT_INFO.querySpAreas", map);
	}
