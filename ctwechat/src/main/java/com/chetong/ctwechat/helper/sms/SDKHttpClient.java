package com.chetong.ctwechat.helper.sms;
		...
		if (null != responseString && !"".equals(responseString)) {
			...
			for (Element element : elements) {
				if (null != element) {
					...
					System.out.println("收到一条报告：手机号码=" + report.getMobile() + "|消息id=" + report.getSeqID() + "|状态="
							+ report.getErrorCode() + "|报告时间=" + report.getReceiveDate());
					_Reports.add(report);
				}
			}
